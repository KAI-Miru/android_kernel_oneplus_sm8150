...
TARGET_CHANGE=775be2311ae448df6eeb027412e836d75211caed
SCAFFOLD=b92a77e96dd54fd30f8f39c7eef23e76f211c515
SCAFFOLD_PARENT1=b125a425ef1559871b1d6cd662806c8afc53e934
PREVIOUS_VALIDATED_HEAD=5c2a198931942c8ac53b27fda2a4c44656eab89c
GLINK_SOURCE=0986f57a7fed5bf69ca14f141666a648a1471350
LEDGER=Documentation/miru/lts-4.14.305-conflicts.md
VENDOR_SHA=125ff7d0153cbb3aaa8f9fd618c33b7f728d7798
DIAG=lts305-pixel-clock-parity
OWNED_PATH=drivers/clk/qcom/clk-rcg2.c
TARGET_OBJECT=drivers/clk/qcom/clk-rcg2.o
EXPECTED_SCAFFOLD_BLOB=9fc98819321a4f00e0c21aed3fcb66f657ad3fb1
...
git merge-base --is-ancestor "${GLINK_SOURCE}" "${START_HEAD}"
...
if grep -Fq '### Qualcomm pixel-clock fraction parity' "${LEDGER}"; then
...
PIXEL_TABLE="$(sed -n '/static const struct frac_entry frac_table_pixel\[\]/,/^};/p' "${OWNED_PATH}")"
test "$(printf '%s\n' "${PIXEL_TABLE}" | grep -Fc '{ 2, 3 },')" = 1
for ratio in '{ 1, 1 },' '{ 4, 9 },' '{ 3, 8 },' '{ 2, 9 },'; do
  printf '%s\n' "${PIXEL_TABLE}" | grep -Fq "${ratio}"
done
{
  echo "result=PASS"
  echo "owned_path=${OWNED_PATH}"
  echo "scaffold_blob=${EXPECTED_SCAFFOLD_BLOB}"
  echo "target_ratio_2_3=present_once"
  echo "downstream_ratios_1_1_4_9_3_8_2_9=present"
...
git commit --allow-empty -m 'lts: confirm Qualcomm pixel-clock fraction parity for 4.14.305'
...
OUT_DIR="${ANDROID_ROOT}/out/h40-pixel-clock-targeted"
...
WORKTREE_TABLE="$(sed -n '/static const struct frac_entry frac_table_pixel\[\]/,/^};/p' "${WORKTREE_PATH}")"
test "$(printf '%s\n' "${WORKTREE_TABLE}" | grep -Fc '{ 2, 3 },')" = 1
...
REVERT_WORKTREE="${RUNNER_TEMP}/lts305-pixel-clock-revert"
...
git -C "${REVERT_WORKTREE}" commit --allow-empty \
  -m 'Revert "lts: confirm Qualcomm pixel-clock fraction parity for 4.14.305"'
...
text = path.read_text()
for old, new in {
    '- Semantically resolved conflicts: **10**': '- Semantically resolved conflicts: **11**',
    '- Remaining semantic conflicts: **23**': '- Remaining semantic conflicts: **22**',
}.items():
    if old not in text:
...
pattern = re.compile(
    r'^(\| 9 \| .*? \| index-resolved in scaffold \| )unresolved( \| — \| — \| — \|)$',
    re.M,
)
match = pattern.search(text)
if not match:
    raise SystemExit('missing unresolved pixel-clock manifest row 9')
replacement = (
...
glink_anchor = '- Validation workflow run: `30240476056`.\n'
glink_evidence = (
    '- Validation artifact: run `30240476056`, artifact `8643145751`, '
    'digest `sha256:77b97379d5b34a933cc86757edd4efe5806eb507af85f2e97f1c299f5b566787`, '
    'size `7883` bytes.\n'
)
if glink_anchor not in text:
    raise SystemExit('missing GLINK validation anchor')
if 'artifact `8643145751`' not in text:
    text = text.replace(glink_anchor, glink_anchor + glink_evidence, 1)
...
- Downstream behavior retained: the Miru table already contains `{ 2, 3 }` exactly once together with `{ 1, 1 }`, `{ 4, 9 }`, `{ 3, 8 }`, and `{ 2, 9 }`; the scaffold blob is `{os.environ.get('EXPECTED_SCAFFOLD_BLOB', '9fc98819321a4f00e0c21aed3fcb66f657ad3fb1')}`.
...
- Clean reversal: **PASS**; reverting the empty semantic commit in a disposable worktree preserved the owned path and restored the exact scaffold tree `{os.environ['SCAFFOLD']}`.
- Validation workflow run: `{os.environ.get('GITHUB_RUN_ID', 'unknown')}`.
'''
if '### Qualcomm pixel-clock fraction parity' in text:
    raise SystemExit('pixel-clock parity record already exists')
text += record
...
git commit -m 'docs: record pixel-clock parity validation [skip ci]'
...
test "$(git rev-parse HEAD^)" = "${SOURCE_COMMIT}"
{
  ...
  echo "semantic_conflicts_resolved=11"
  echo "semantic_conflicts_remaining=22"
