...
# Correct the generated build driver's stale scaffold assertion. The current
# local merge commit, not the 4.14.291 source scaffold, must have stable 292 as
# its second parent.
...
grep -Fq "parent ${STABLE_SHA}" "${DIAG_DIR}/merge-scaffold.txt"'''
if s.count(old) != 1:
    raise SystemExit('stale build-driver scaffold gate changed unexpectedly')
...
# Execute the generated driver through its source sanity gate, then stop before
# apt, workspace recreation, toolchain download, or compilation.
...
trap 'on_error ${LINENO}' ERR
"""
driver_preflight = r'''python3 - <<'DRIVERSTOP'
from pathlib import Path
p = Path('scripts/miru/ci_build_4.14.190.sh')
s = p.read_text()
