...
struct uid_entry {
	...
	atomic64_t cg_checkout[UID_GROUP_SIZE];
#endif
};
...
static int uid_perf_enable_store(const char *buf, const struct kernel_param *kp)
{
	...
	if (val != 0 && val != 1)
		return -EINVAL;
	if (!uid_perf_add_thread || !uid_perf_remove_thread)
		return -ENODEV;
	...
	if (val && uid_perf_add_thread)
		wake_up_process(uid_perf_add_thread);
	else if (uid_perf_remove_thread)
		wake_up_process(uid_perf_remove_thread);
...
static int __init proc_uid_sys_stats_init(void)
{
	...
	uid_perf_add_thread = kthread_run(__uid_perf_add_work, NULL,
		"uid_add_thread");
	if (IS_ERR(uid_perf_add_thread)) {
		pr_err("%s: failed to start uid_add_thread\n", __func__);
		uid_perf_add_thread = NULL;
		goto err;
	}
	uid_perf_remove_thread = kthread_run(__uid_perf_remove_work, NULL,
		"uid_remove_thread");
	if (IS_ERR(uid_perf_remove_thread)) {
		pr_err("%s: failed to start uid_remove_thread\n", __func__);
