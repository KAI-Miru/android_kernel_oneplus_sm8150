...
static int get_cpuset_cgrp_idx(int cgid)
{
	...
	hash_for_each_possible(cg_hash_table, cg, hash, cgid) {
		if (cg->cgid == cgid) {
			idx = cg->idx;
			break;
		}
...
void cpuset_add_cg(int cgid, char *name)
{
	...
	hash_for_each_possible(cg_hash_table, cg, hash, cgid)
		if (cg->cgid == cgid)
			return;
	if (cg_entry_count >= UID_GROUP_SIZE)
		return;

	cg = kzalloc(sizeof(*cg), GFP_ATOMIC);
	if (!cg)
		return;

	cg->cgid = cgid;
	cg->idx = cg_entry_count++;
...
#ifdef CONFIG_OPLUS_FEATURE_UID_PERF
	hash_init(cg_hash_table);
	cpu_parent = proc_mkdir("cpuset_info", NULL);
	if (!cpu_parent) {
		pr_err("%s: failed to create cpuset_info proc entry\n",
		       __func__);
	} else {
		proc_create_data("show_hash", 0444, cpu_parent,
				 &hashdump_fops, NULL);
		proc_create_data("dbg_enable", 0644, cpu_parent,
				 &dbg_fops, NULL);
	}
#endif
